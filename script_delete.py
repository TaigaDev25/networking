...
2. Read a "delete configuration" snippet (XML) from a file.
3. Connect to each device over NETCONF (PyEZ).
4. Retrieve current config in XML and save it (for backup).
5. Push the "delete" XML snippet to remove part of the config and commit changes atomically.
"""
...
def delete_config_on_device(device_params, delete_snippet):
    """
    Connects to the device, locks config, loads the 'delete_snippet' (XML),
    commits changes, handles errors with rollback, then unlocks.
    This snippet should contain operation="delete" in the XML to remove config.
    """
    dev_params = {
        'host': device_params['host'],
        'user': device_params.get('user', 'root'),
    ...
    try:
        with Device(**dev_params) as dev:
            ...
            # This snippet is expected to have the required <operation="delete"> lines
            cfg.load(delete_snippet, format='xml', merge=True)
            print(f"[INFO] Loaded 'delete' snippet on {log_host}.")
            ...
            # Commit
            cfg.commit()
            print(f"[INFO] Configuration deletion committed on {log_host}.")
...
def main():
    ...
    - Read "delete configuration" snippet from an XML file (second argument).
    - For each device:
        - Retrieve XML config and save locally (backup).
        - Push the "delete" XML snippet and commit.
    """
    if len(sys.argv) < 3:
        print("Usage: python script_delete.py <devices.json> <config_delete_snippet.xml>")
        sys.exit(1)

    json_file = sys.argv[1]
    delete_snippet_file = sys.argv[2]
    ...
    # 2. Read XML snippet to delete config
    try:
        with open(delete_snippet_file, 'r', encoding='utf-8') as f:
            delete_snippet = f.read()
    except FileNotFoundError:
        print(f"[ERROR] XML file '{delete_snippet_file}' not found.")
        sys.exit(1)
    except OSError as e:
        print(f"[ERROR] Could not read XML file '{delete_snippet_file}': {e}")
        sys.exit(1)

    # 3. For each device, retrieve config -> save -> delete snippet
    for dev_params in devices:
        ...
        delete_config_on_device(dev_params, delete_snippet)
